...
export const PlayersManager: React.FC<PlayersManagerProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-gramado-card via-gramado-card/95 to-gramado p-6 rounded-3xl border border-gramado-light shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className="px-5 py-2.5 bg-capim hover:bg-capim text-giz rounded-2xl text-xs font-black flex items-center gap-2 shadow-lg shadow-capim/20 transition-all"
          >
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-gramado-card/80 p-4 rounded-2xl border border-gramado-light">
        <div className="relative flex-1">
          <Search className="w-4 h-4 text-giz/50 absolute left-3.5 top-1/2 -translate-y-1/2" />
          <input
            type="text"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            placeholder="Buscar por nome ou apelido..."
            className="w-full bg-gramado border border-giz/15 rounded-xl pl-9 pr-3 py-2 text-xs text-giz focus:outline-none focus:border-capim"
          />
        </div>
            ...
            {['all', 'GK', 'DEF', 'MID', 'ATT'].map((pos) => (
              <button
                ...
                className={`px-2.5 py-1 rounded-lg font-bold transition-all ${
                  positionFilter === pos ? 'bg-capim text-giz' : 'text-giz/50 hover:text-giz'
                }`}
              >
            ...
            {['all', 'mensalista', 'diarista'].map((tp) => (
              <button
                ...
                className={`px-2.5 py-1 rounded-lg font-bold capitalize transition-all ${
                  typeFilter === tp ? 'bg-capim text-giz' : 'text-giz/50 hover:text-giz'
                }`}
              >
                {tp === 'all' ? 'Todos' : tp}
              </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {filteredPlayers.length === 0 ? (
          <div className="col-span-full py-16 text-center bg-gramado-card/60 rounded-3xl border border-dashed border-gramado-light p-8">
            <Users className="w-12 h-12 text-giz/25 mx-auto mb-3" />
            <h3 className="text-lg font-bold text-giz mb-1">Nenhum jogador encontrado</h3>
            ...
            {players.length === 0 && (
              <button
                onClick={() => setIsAddModalOpen(true)}
                className="px-5 py-2.5 bg-capim hover:bg-capim text-giz rounded-xl font-bold text-xs inline-flex items-center gap-2"
              >
                <UserPlus className="w-4 h-4" />
      ...
      {isAddModalOpen && (
        ...
        >
          <div
            className="w-full max-w-lg bg-gramado-card border border-gramado-light rounded-3xl p-6 shadow-2xl my-8"
          ...
          >
            ...
            <form onSubmit={handleCreatePlayer} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-giz/70 font-bold block mb-1">Apelido na Pelada</label>
                  <input
                    type="text"
                    value={nickname}
                ...
                <div>
                  <label className="text-giz/70 font-bold block mb-1">Tipo de Membro</label>
                  <select
                    value={type}
                ...
                <div>
                  <label className="text-giz/70 font-bold block mb-1">WhatsApp</label>
                  <input
                    type="text"
                    value={phone}
                  ...
                  {[
                    ...
                    <div key={attr.label}>
                      <div className="flex justify-between text-giz/70 font-bold mb-1">
                        ...
                        <span className="text-capim-light font-black">{attr.val}</span>
                      </div>
                      <input
                        type="range"
                        min="30"
